<?php
   ...
   function form_validation()
   {
       if($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['btn_signup']))
       {
           ...
           // Check Empty Fields
           if(empty($FirstName) || empty($LastName) || empty($UserName) || empty($Email) || empty($Password) || empty($CPassword)|| empty($Phone))
           {
           ...
           if($Password!=$CPassword)
           {
            ...
            if(!preg_match('@[A-Z]@', $Password)) {
                $Errors[] = "Password should contain atleast one UPPERCASE character";
            }
            
            if(!preg_match('@[a-z]@', $Password)) {
                $Errors[] = "Password should contain atleast one lowercase character";
            } 
            if(!preg_match('@[^\w]@', $Password)) {
                $Errors[] = "Password should contain atleast one special character";
            } 
            if(!preg_match('@[0-9]@', $Password)) {
                $Errors[] = "Password should contain atleast one numeric character";
            } 
           ...
           else
           {
               ...
               if(user_registration($FirstName,$LastName,$UserName,$Email,$Password,$Phone))
               {
   ...
   function form_validationcaptcha()
   {
       if($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['g-recaptcha-response']))
       {
           ...
           $Errors=[];
           $secret = '6LfyYGMjAAAAAO88gbyqZbVZTSKX5dRiuLDFYnvn';
   ...
   function user_registration($First,$Last,$User,$Email,$Password,$Phone)
   {
        global $con;
        ...
        if (file_exists($filename) && file_exists($filename2)) {
          $sql = "insert into users (FirstName,LastName,UserName,Email,Password,Validation_Code,Active,Phone) values ('$FirstName','$LastName','$UserName','$Email','$Pass','$Validation_Code','0','$Phone')";
        ...
        $Msg = " Please Active Your Account to Click the Link Below : https://lovejoyantiquessecurity.000webhostapp.com/activate.php?Email=$Email&Code=$Validation_Code";
   ...
   function login_validation()
   {
       if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['btn-login']))
       {
           ...
           {
               if(login($Email,$password,$Remember) && is_admin())
               {
   ...
   function is_admin(){
    if(logged_in()&& $_SESSION['Email']=="theowneroflj@gmail.com"){
        return true;
    }
    else{
        return false;
      ...
      if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['btn_recover']))
      {
          if(isset($_SESSION['token']) && $_POST['token_input'] == $_SESSION['token'])
          {
                ...
                if(Email_Exist($Email))
                {
                    ...
                    $Message = " Your Reset Validation Code is {$code} please click here https://lovejoyantiquessecurity.000webhostapp.com/reset.php?Email='$Email'&Code='$code'";
                    $header = "no-reply@onlineittuts.com";
